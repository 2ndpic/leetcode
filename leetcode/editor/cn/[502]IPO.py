...
#  0 <= capital[i] <= 10⁹ 
#  
#  Related Topics 贪心 数组 排序 堆（优先队列） 👍 181 👎 0
...
class Solution:
    def findMaximizedCapital(self, k: int, w: int, profits: List[int], capital: List[int]) -> int:
        ...
        cap_pro = sorted(zip(capital, profits), key=lambda x: x[0])
        pq, cur, n = [], 0, len(cap_pro)
        for _ in range(k):
            while cur < n and w >= cap_pro[cur][0]:
                heapq.heappush(pq, -cap_pro[cur][1])
                cur += 1
