    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(x, y, path, start):
            ...
            for dx, dy in ((-1, 0), (0, 1), (1, 0), (0, -1)):
                nx, ny = x + dx, y + dy
                if 0 <= nx < m and 0 <= ny < n and board[nx][ny] == path[start] and not visited[nx][ny]:
                    visited[nx][ny] = True
                    tmp = dfs(nx, ny, path, start + 1)
                    visited[nx][ny] = False
                    if tmp: return True
            return False
