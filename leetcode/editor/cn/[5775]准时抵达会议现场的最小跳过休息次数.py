...
class Solution:
    def minSkips(self, dist: List[int], speed: int, hoursBefore: int) -> int:
        """
        f[i][j]表示经过dist[i-1]时跳过j次的最短用时,
        注意经过dist[0]最多跳0次，经过dist[1]最多跳1次,经过dist[i-1]最多跳i-1次，so j < i
        考虑两种情况,dist[i-2]经过dist[i-1]跳了的和没跳
        - dist[i-2]经过dist[i-1]跳了, 用时 f[i - 1][j - 1] + dist[i - 1] / speed
        - dist[i-2]经过dist[i-1]没跳, 用时 ceil(f[i - 1][j]) + dist[i - 1] / speed
        so f[i][j] = min(跳了，没跳)
        初始化: f[0][0] = 0
        """
        eps = 1e-8
        ...
        for i in range(1, n + 1):
            for j in range(i):
                ...
                if i == 1 or j != i - 1:
                    f[i][j] = min(f[i][j], ceil(f[i - 1][j] - eps) + dist[i - 1] / speed)
        ans = n
        for i in range(n):
            if f[n][i] - eps <= hoursBefore and i < ans:
                ans = i
