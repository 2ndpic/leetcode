...
#  
#  Related Topics 深度优先搜索 广度优先搜索 数组 哈希表 👍 63 👎 0
...
class Solution:
    def isEscapePossible(self, blocked: List[List[int]], source: List[int], target: List[int]) -> bool:
        ...
        BLOCKED: 在包围圈中
        VALID: 不在包围圈中
        FOUND: 无论在不在包围圈中，但在n * (n - 1) / 2步搜索的过程中经过了target
        """
        def check(start, finish):
            ...
            while q and countdown > 0:
                ...
                for nx, ny in [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]:
                    if 0 <= nx < BOUNDARY and 0 <= ny < BOUNDARY and (nx, ny) not in hash_blocked and (nx, ny) not in vis:
                        if (nx, ny) == (fx, fy):
                            return FOUND
                        countdown -= 1
                        q.append((nx, ny))
        ...
        BOUNDARY = 10 ** 6
        if len(blocked) < 2: return True
        hash_blocked = set(tuple(pos) for pos in blocked)

        if (ans := check(source, target)) == FOUND: return True
        elif ans == BLOCKED: return False
        else:
            ans = check(target, source)
            if ans == BLOCKED: return False
            return True
