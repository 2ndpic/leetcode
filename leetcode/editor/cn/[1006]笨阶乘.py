...
class Solution:
    def clumsy(self, N: int) -> int:
        '''
        首先直接暴力循环的话，观察到数据规模是很大的，python很可能会超时
        观察到一个一个循环主要处理的是乘除，加减法都可以根据交换律移位置
        N * (N - 1)/(N - 2) = (N - 2 + 2)(N - 2 + 1)/(N - 2) = N + 1 + 2 / N - 2
        N >= 5开始， N * (N - 1) / (N - 2) = N + 1
        用N=10举例：笨阶乘就相当于：11+7-7+3-2*1
        可以观察到中间的项数都被抵消掉了

        最后的工作就是处理余项了，非常简单
        '''
        if N <= 4:
            return [None, 1, 2, 6, 7][N]
        if N > 4:
            return N + 1 + [0, 1, 1, -2][N%4]
